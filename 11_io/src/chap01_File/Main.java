 ...
 *      (1) File.separator 필드를 활용합니다. (운영체제를 따라서 자동으로 경로 구분자가 선택됩니다.)
 *      (2) Window에서 개발할때도 슬래스(/)를 사용합니다. (대부분 슬래시, 역슬래시 모두 경로 구분자로 인식하도록 호환성을 제공합니다.)
 * 2. 예약된 디렉터리
 *    1) 현재 디렉터리 : .
 *    2) 상위 디렉터리 : ..
 *    3) 루트 디렉터리 : /
...
public class Main {
  ...
  public static void handling() {
    ...
    File dir2 = new File("D:/" + DateTimeFormatter.ofPattern("yyyy/MM/dd").format(LocalDateTime.now()));
