...
public class Customer {
  ...
  public String purchase() {
    ...
    int total = 0;                               // 구매 총액
    for (int i = 0; i < count; i++) {           // 구매한 Product 개수만큼 반복     // for (int i = 0, cnt = cart.getCount(); i < cnt; i++) : 좀더 좋은 코드
      Map<String, Object> product = products.get(i);             // 구매한 개별 Product              // cart.getProducts(): 배열, products와 동일
      int price = (int) product.get("price");           // 개별 Product의 가격
      total += price;                           // 개별Product 가격을 구매총액에 누적
