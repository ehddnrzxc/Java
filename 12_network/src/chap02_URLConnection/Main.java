 ...
 * 4. HTTP 프로토골의 경우 HttpURLConnection 클래스가 하위 클래스로 지원됩니다.
...
public class Main {
  ...
  public static void byteStream() throws MalformedURLException, IOException {
    ...
    int resopnseCode = con.getResponseCode();  // HTTP 상태코드 (200: OK, 404: Not Found 등)
    
    BufferedInputStream in = null;   // 이미지 서버로부터 데이터를 입력 받는 버퍼스트림
    
    if (resopnseCode == HttpURLConnection.HTTP_OK) {      // if (responseCode == 200) { 이 코드와 같은 의미
    ...
    int readByte = 0;    // 실제로 읽은 데이터의 바이트 
